...
class ThreeColumnEngine(OcrEngine):
    def is_available(self) -> bool:
        return True
    ...
    def recognize(self, image: Image.Image) -> list[RecognizedLine]:
        ...
        return [
            RecognizedLine(
                text=text,
                bbox=BoundingBox(x=x, y=y, width=130, height=30),
                confidence=0.95,
                block_id=index,
                paragraph_id=index,
                line_id=index,
...
def test_pipeline_uses_three_column_reading_order(tmp_path) -> None:
    source = tmp_path / "three-columns.png"
    Image.new("RGB", (600, 360), "white").save(source)

    document = process_document(source, engine=ThreeColumnEngine())
